...
テストデータにつき、削除していただいてかまいません。','draft')";
  ...
  //magazineTransferPrice
  $fp = fopen(FULLPATH.'magazineTransferPrice.txt', "w");
  @fwrite($fp, $_POST['amount'], strlen($_POST['amount']) );
  fclose($fp);
  
  //magazineTransferDeadline
  $fp = fopen(FULLPATH.'magazineTransferDeadline.txt', "w");
  @fwrite($fp, $_POST['deadline'], strlen($_POST['deadline']) );
  fclose($fp);
  
  //magazineTransferCheck
  $fp = fopen(FULLPATH.'magazineTransferCheck.txt', "w");
  @fwrite($fp, $_POST['check'], strlen($_POST['check']) );
  fclose($fp);
